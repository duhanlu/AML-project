...
print("Original labels: ", trainy[:5])
# plot 25 images
for i in range(25):
	plt.subplot(5, 5, 1 + i)
	plt.axis('off')
...
def load_real_samples():
	...
	# convert to floats and scalefeatures = pd.read_csv("features.csv")
	kmeans = KMeans(n_clusters=2, random_state=0, n_init="auto").fit(features)
	labels = pd.DataFrame(kmeans.labels_).to_numpy()
	trainy = labels
	print("The generated labels: ", trainy[:5])
...
def train(g_model, d_model, gan_model, dataset, latent_dim, n_epochs=100, n_batch=128):
	...
	loss_df = pd.DataFrame(['bat_per_epo','d_loss_real','d_loss_fake','g_loss']) 
	columns =  ['bat_per_epo','d_loss_real','d_loss_fake','g_loss']                 #and a half batch of fake samples, combined a single batch. 
	...
	for i in range(n_epochs):
        ...
        #job of folling the discriminator then the output would be 1 (true)
			...
			new_values = [bat_per_epo, d_loss_real, d_loss_fake, g_loss]
			loss_df.loc[i+1] = new_values
			# Print losses on this batch
			print('Epoch>%d, Batch%d/%d, d1=%.3f, d2=%.3f g=%.3f' %(i+1, j+1, bat_per_epo, d_loss_real, d_loss_fake, g_loss))
	#save the model and loss df	
	loss_df.to_csv('loss.csv',index = True)
	model.save('cifar_conditional_generator_25epochs.h5')
